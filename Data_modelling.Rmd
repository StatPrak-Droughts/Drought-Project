...
hydro_winter_10304$avg30_snowstorage <- zoo::rollmean(hydro_winter_10304$snowstorage, k = 30, fill = mean(hydro_winter_10304$snowstorage))
...
hydro_summer_10304$avg30_snowstorage <- zoo::rollmean(hydro_summer_10304$snowstorage, k = 30, fill = mean(hydro_summer_10304$snowstorage))
...
for (i in pegel_ids) {
  for (j in levels(hydro_total$member)) {
    snowstorage_drain <- get(paste0("hydro_winter_", i, "_", j))$snowstorage_diff
    for (k in 1:length(snowstorage_drain)) {
      if (!is.na(snowstorage_drain[k])) {
        if (snowstorage_drain[k] >= 0) {
        snowstorage_drain[k] <- NA
        }
      }
    }
    assign(paste0("hydro_winter_", i, "_", j), cbind(get(paste0("hydro_winter_", i, "_", j)), snowstorage_drain))  
  }
}
...
for (i in pegel_ids) {
  for (j in levels(hydro_total$member)) {
    snowstorage_drain <- get(paste0("hydro_summer_", i, "_", j))$snowstorage_diff
    for (k in 1:length(snowstorage_drain)) {
      if (!is.na(snowstorage_drain[k])) {
        if (snowstorage_drain[k] >= 0) {
        snowstorage_drain[k] <- NA
    ...
    assign(paste0("hydro_summer_", i, "_", j), cbind(get(paste0("hydro_summer_", i, "_", j)), snowstorage_drain)) 
  }
}
...
for (i in pegel_ids) {
  for (j in levels(hydro_total$member)) {
    avg_snowstorage_drain <- as.numeric(rep(NA, length = nrow(get(paste0("hydro_winter_", i, "_", j)))))
    for (k in 7:length(avg_snowstorage_drain)) {
      if (get(paste0("hydro_winter_", i, "_", j))$MM[k] == 11) {
        if (all(get(paste0("hydro_winter_", i, "_", j))$MM[(k - 6):k] == 11)) {
           avg_snowstorage_drain[k] <- mean(get(paste0("hydro_winter_", i, "_", j))$snowstorage_drain[(k - 6):k],
                                             na.rm = TRUE)
        }
      } else {
        avg_snowstorage_drain[k] <- mean(get(paste0("hydro_winter_", i, "_", j))$snowstorage_drain[(k - 6):k],
    ...
    assign(paste0("hydro_winter_", i, "_", j), cbind(get(paste0("hydro_winter_", i, "_", j)), avg_snowstorage_drain))  
  }
}
...
for (i in pegel_ids) {
  for (j in levels(hydro_total$member)) {
    avg_snowstorage_drain <- as.numeric(rep(NA, length = nrow(get(paste0("hydro_summer_", i, "_", j)))))
    for (k in 7:length(avg_snowstorage_drain)) {
      if (get(paste0("hydro_summer_", i, "_", j))$MM[k] == 5) {
        if (all(get(paste0("hydro_summer_", i, "_", j))$MM[(k - 6):k] == 5)) {
           avg_snowstorage_drain[k] <- mean(get(paste0("hydro_summer_", i, "_", j))$snowstorage_drain[(k - 6):k],
                                             na.rm = TRUE)
        }
      } else {
        avg_snowstorage_drain[k] <- mean(get(paste0("hydro_summer_", i, "_", j))$snowstorage_drain[(k - 6):k],
    ...
    assign(paste0("hydro_summer_", i, "_", j), cbind(get(paste0("hydro_summer_", i, "_", j)), avg_snowstorage_drain))  
  }
}
...
```
# AIC comparison for member kbe
```{r}
hydro_summer_10304_kbe <- hydro_summer_10304 %>% filter(member %in% "kbe")
hydro_summer_11502_kbe <- hydro_summer_11502 %>% filter(member %in% "kbe")
hydro_summer_20203_kbe <- hydro_summer_20203 %>% filter(member %in% "kbe")

hydro_summer_10304_kbe$avg30_snowstorage <- zoo::rollmean(hydro_summer_10304_kbe$snowstorage, k = 30, fill = mean(hydro_summer_10304_kbe$snowstorage))
hydro_summer_11502_kbe$avg30_snowstorage <- zoo::rollmean(hydro_summer_11502_kbe$snowstorage, k = 30, fill = mean(hydro_summer_11502_kbe$snowstorage))
hydro_summer_20203_kbe$avg30_snowstorage <- zoo::rollmean(hydro_summer_20203_kbe$snowstorage, k = 30, fill = mean(hydro_summer_20203_kbe$snowstorage))
```

```{r}
hydro_winter_10304_kbe <- hydro_winter_10304 %>% filter(member %in% "kbe")
hydro_winter_11502_kbe <- hydro_winter_11502 %>% filter(member %in% "kbe")
hydro_winter_20203_kbe <- hydro_winter_20203 %>% filter(member %in% "kbe")

hydro_winter_10304_kbe$avg30_snowstorage <- zoo::rollmean(hydro_winter_10304_kbe$snowstorage, k = 30, fill = mean(hydro_winter_10304_kbe$snowstorage))
hydro_winter_11502_kbe$avg30_snowstorage <- zoo::rollmean(hydro_winter_11502_kbe$snowstorage, k = 30, fill = mean(hydro_winter_11502_kbe$snowstorage))
hydro_winter_20203_kbe$avg30_snowstorage <- zoo::rollmean(hydro_winter_20203_kbe$snowstorage, k = 30, fill = mean(hydro_winter_20203_kbe$snowstorage))
```
